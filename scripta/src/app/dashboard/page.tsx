    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <StatCard
          ...
          value={plan === 'pro' ? '∞' : credits.toString()}
          icon={Zap}
          accent="bg-amber-500"
        />
        <StatCard
            ...
            {MODES.map((mode) => {
              ...
              return (
                <Link
                  ...
                  className="group rounded-xl border border-border/60 bg-card p-4 hover:shadow-theme-md hover:border-primary-300 transition relative card-lift"
                >
                  <div className="flex items-center justify-between">
                    <div
                      className={`grid h-10 w-10 place-items-center rounded-lg bg-gradient-to-br ${mode.color} text-white shadow-theme-sm`}
                    >
                  ...
                  <h3 className="mt-3 font-semibold text-sm">{mode.name}</h3>
                  <p className="mt-1 text-xs text-muted-foreground line-clamp-2">
                    {mode.desc}
                  </p>
                  <div className="mt-3 flex items-center gap-1 text-xs font-medium text-primary-600 opacity-0 group-hover:opacity-100 transition">
                    Open
            ...
            <div className="divide-y divide-border/40">
              {recentDocs.map((doc) => (
                <Link
                  ...
                  className="flex items-center gap-4 py-3 group hover:bg-muted/40 -mx-2 px-2 rounded-md transition"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium truncate group-hover:text-primary-600 transition">
                      {doc.title}
                    </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-2 h-1.5 w-full rounded-full bg-muted overflow-hidden">
                      <div
                        className="h-full button-bg rounded-full transition-all"
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
  ...
  return (
    <div className="rounded-xl border border-dashed border-border/60 py-12 px-6 text-center">
      <div className="mx-auto max-w-sm">
        <svg
          viewBox="0 0 64 64"
          className="mx-auto h-16 w-16 text-primary-400/60"
          fill="none"
          stroke="currentColor"
          strokeWidth="1.5"
        >
        ...
        <Button
          asChild
          className="mt-4 button-bg btn-press text-white h-10"
        >
          <Link href="/dashboard/write/blog">
            <Sparkles className="mr-2 h-4 w-4" />
            Start writing
