...
import {
  BillingDashboard,
  type BillingRow,
} from "@/components/driftframe/billing-dashboard";
...
export default async function BillingPage() {
  ...
  // Build a unified transaction list with invoice metadata for purchases.
  const purchaseByDate = new Map(
    purchases.map((p) => [p.createdAt.getTime(), p]),
  );
  ...
  const rows: BillingRow[] = transactions.map((tx) => {
    ...
    return {
      ...
      invoiceId: matchingPurchase?.stripePaymentIntentId,
  ...
  const purchaseRows: BillingRow[] = purchases.map((p) => ({
    ...
    invoiceId: p.stripePaymentIntentId,
